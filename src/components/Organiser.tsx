...
import {RecordSource} from '../lib/profiles';
...
export default function Organiser({initialPage='Home'}:{initialPage?:string}) {
  return <ProfileHub>{(source,toolbar,identity)=><Records key={identity} initialPage={initialPage} source={source} toolbar={toolbar}/>}</ProfileHub>;
}
function Records({initialPage,source,toolbar}:{initialPage:string;source:RecordSource;toolbar:React.ReactNode}) {
  ...
  useEffect(()=>{
    if(!source.shared||edit||profile||busy)return;
    let alive=true;let checking=false;
    async function refresh(){
      if(checking||writing.current)return;checking=true;
      try{const next=await source.load();if(alive){current.current=next;setState(next);setReady(true);setError('');}}
      catch(e){if(alive){current.current=model.empty();setState(model.empty());setReady(false);setError(e instanceof Error?e.message:'Unable to check shared access.');}}
    ...
    // Recheck membership even when removal prevents delivery of Realtime events.
    const timer=setInterval(()=>{void refresh();},30000);
    return()=>{alive=false;unsubscribe?.();listener.remove();clearInterval(timer);};
  },[source,edit,profile,busy]);
  ...
  async function commit(s:State){await source.save(s);current.current=s;setState(s);}
  function go(p:string){setPage(p);setFilter('All');setSearch('');setNotice('');}
  function add(kind=page){if(!source.admin)return;setEdit(model.blank(model.kinds.includes(kind)?kind:'Note'));setError('');}
  ...
  const nav=<BottomNav canAdd={source.admin} active={page} onSelect={p=>p==='Add'?add('Note'):go(p)}/>;
  if(!ready)return <Shell nav={null}>{toolbar}<Label variant="title">Inside & Out</Label><Label>{error||'Loading your organiser…'}</Label>{!!error&&<Action disabled={busy} onPress={()=>work(async()=>{const next=await source.load();current.current=next;setState(next);setReady(true);})}>Try again</Action>}</Shell>;
  return <Shell nav={nav}>
    ...
    {toolbar}
    {source.shared&&<Box variant="row"><Label variant="small">Shared family records · online access required</Label><Action disabled={busy||!!edit||!!profile} tone="quiet" onPress={()=>work(async()=>{const next=await source.load();current.current=next;setState(next);setNotice('Latest family updates loaded.');})}>Refresh updates</Action></Box>}
    ...
    {page==='Home'&&<>
      <ImageBackground source={require('../../assets/inside-out-mountains.png')} imageStyle={ui.welcomeImage} style={ui.welcome}>
        <View style={ui.welcomeCopy}><Text style={ui.eyebrow}>YOUR ORGANISER</Text><Text style={ui.welcomeTitle}>{state.profile.name?`Supporting ${displayName(state.profile.name).split(' ')[0]}`:'A clearer path forward'}</Text><Text style={ui.quote}>Small steps keep bigger futures possible.</Text></View>
      </ImageBackground>
      <Pressable disabled={!source.admin||busy} accessibilityRole="button" onPress={()=>setProfile({...state.profile})} style={ui.profileStrip}>
      ...
      {issues.length>0&&<Box variant="alert"><Label variant="badge">NEEDS FOLLOW-UP · {issues.length}</Label><Label variant="heading">{issues[0].title}</Label><Label variant="small">{issues[0].date} · {issues[0].status}</Label><Action tone="quiet" onPress={()=>{go(issues[0].kind);if(source.admin)setEdit({...issues[0]});}}>View record</Action></Box>}
      ...
      {source.admin&&!source.shared&&!entries.length&&!state.profile.name&&<Action tone="quiet" disabled={busy} onPress={()=>work(async()=>{await commit(model.sample());})}>Explore sample records</Action>}
    </>}
    {(model.kinds.includes(page)||page==='Timeline')&&<>
      {page==='Task'&&<Box variant="row"><Label variant="badge">{entries.filter(e=>e.kind==='Task'&&e.status!=='Completed').length} TO DO</Label><Label variant="badge">{entries.filter(e=>e.kind==='Task'&&e.status==='Completed').length} COMPLETED</Label></Box>}
      ...
      {visible.map(e=><Box variant="card" key={e.id}><Label variant="badge">{e.kind} · {e.status}</Label><Label variant="heading">{e.title}</Label><Label variant="small">{formatDate(e.date,e.time)}{e.place?' · '+e.place:''}</Label>{!!e.details&&<Label>{e.details}</Label>}{e.updatedBy&&<Label variant="small">Updated by {e.updatedBy}{e.updatedAt?' · '+new Date(e.updatedAt).toLocaleString('en-AU'):''}</Label>}{e.kind==='Task'&&<Action disabled={busy} tone="teal" onPress={()=>work(async()=>{if(source.completeTask){const next=await source.completeTask(e.id,e.status!=='Completed',current.current.cloudRevision);current.current=next;setState(next);}else{await commit({...current.current,entries:current.current.entries.map(x=>x.id===e.id?{...x,status:e.status==='Completed'?'Planned':'Completed'}:x)});}setNotice('Task status saved.');})}>{e.status==='Completed'?'Reopen task':'Mark completed'}</Action>}<Box variant="row">{source.admin&&<Action tone="quiet" disabled={busy} onPress={()=>{setError('');setEdit({...e});}}>Edit</Action>}{e.attachment&&<Action tone="quiet" disabled={busy} onPress={()=>work(async()=>{await platform.open(e.attachment!);})}>Open / share</Action>}{source.admin&&<Action tone="quiet" disabled={busy} onPress={()=>setConfirm(e)}>Delete</Action>}</Box>{!source.shared&&platform.native&&e.kind!=='Document'&&e.status!=='Completed'&&<Action tone="quiet" disabled={busy} onPress={()=>work(async()=>{if(e.reminderId){await platform.cancelReminder(e.reminderId);await commit({...current.current,entries:current.current.entries.map(x=>x.id===e.id?{...x,reminderId:undefined}:x)});setNotice('Reminder cancelled.');}else{const id=await platform.remind(e);try{await commit({...current.current,entries:current.current.entries.map(x=>x.id===e.id?{...x,reminderId:id}:x)});}catch(error){await platform.cancelReminder(id);throw error;}setNotice('Reminder scheduled for '+formatDate(e.date,e.time)+'.');}})}>{e.reminderId?'Cancel reminder':'Remind me at this time'}</Action>}</Box>)}
      {page==='Timeline'&&<Box variant="row"><Action tone="quiet" disabled={busy} onPress={()=>work(async()=>{await platform.exportTimeline({...state,entries:visible},'pdf');setNotice('Timeline PDF ready to save or share.');})}>Export PDF</Action><Action tone="quiet" disabled={busy} onPress={()=>work(async()=>{await platform.exportTimeline({...state,entries:visible},'csv');setNotice('Timeline CSV ready to save or share.');})}>Export CSV</Action></Box>}
    </>}
    ...
    {page==='More'&&<><Box variant="card"><Label variant="heading">Your data</Label><Label>{source.shared?'Shared with invited family members. Each inmate has one admin.':platform.storageNote}</Label>{!platform.native&&<Label variant="small">This browser version has no app lock. Use a private device. Clearing browser data can remove saved records and files. Native Face ID and reminders are only in the mobile build.</Label>}<Action disabled={busy} onPress={()=>work(async()=>{await platform.exportBackup(state);})}>Export this inmate’s backup (JSON)</Action>{!source.shared&&<Action disabled={busy} tone="danger" onPress={()=>setConfirm('clear')}>{state.demo?'Clear sample data':'Clear this inmate’s device records'}</Action>}</Box><Box variant="card"><Label variant="heading">About Inside & Out</Label><Label>A family organiser for court, medical, visits, calls and documents.</Label><Label variant="small">Switch between inmate profiles above. Device profiles stay on this phone. Shared profiles require a connected family account. There are no live government feeds.</Label></Box></>}
    ...
    <Sheet open={!!confirm} title="Confirm deletion" onClose={()=>{if(!busy)setConfirm(null);}}><Label>{confirm==='clear'?'Clear this inmate’s device profile, records and attached copies? Other inmate profiles will remain. This cannot be undone. Save a backup and original documents first. Your native app-lock credentials are kept.':'Delete this record and its attached copy? This cannot be undone.'}</Label>{!!error&&<Label variant="error">{error}</Label>}<Action tone="danger" disabled={busy} onPress={deleteConfirmed}>Delete permanently</Action></Sheet>
